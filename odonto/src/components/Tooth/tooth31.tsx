import { Container, Svg, Path } from './styles';

export default function Tooth1(props: any) {
  return (
    <Container {...props}>
      <Svg width="50" height="110" viewBox="0 0 50 110" fill="none" xmlns="http://www.w3.org/2000/svg">
        <Path d="M7.5 86C5 73.5 7.5 43 7.5 42.5C19.5 51.5 35 45 42 45C44 47.5 41 82 39 94C37 106 34 103.5 31 97.5C28 91.5 31 74 28 69C25 64 22 68 18.5 74C15 80 20.5 103.5 16.5 108C12.5 112.5 10 98.5 7.5 86Z" fill="white" />
        <Path d="M2 11C0.8 0.600001 12.5 0.666668 18.5 2C25.5 1 23.5 2 35 2C46.5 2 49.5 9 48.5 13.5C47.7 17.1 46.8333 26.6667 46.5 31C47 34.3333 44.7 41.3 31.5 42.5C15 44 9.5 38 7.5 36.5C5.49999 35 3.5 24 2 11Z" fill="white" />
        <Path d="M7.5 86C5 73.5 7.5 43 7.5 42.5C19.5 51.5 35 45 42 45C44 47.5 41 82 39 94C37 106 34 103.5 31 97.5C28 91.5 31 74 28 69C25 64 22 68 18.5 74C15 80 20.5 103.5 16.5 108C12.5 112.5 10 98.5 7.5 86Z" stroke="black" stroke-width="2" />
        <Path d="M2 11C0.8 0.600001 12.5 0.666668 18.5 2C25.5 1 23.5 2 35 2C46.5 2 49.5 9 48.5 13.5C47.7 17.1 46.8333 26.6667 46.5 31C47 34.3333 44.7 41.3 31.5 42.5C15 44 9.5 38 7.5 36.5C5.49999 35 3.5 24 2 11Z" stroke="black" stroke-width="2" />
      </Svg>

    </Container>
  );
}
